...
class Link {
    ...
    async create(link, sharedContext = {}) {
        ...
        for (const link of allLinks) {
             ...
             * relationship.
             */
            const { linksToValidateForUniqueness, uniquePairsInBatch } = serviceLinks.get(service.__definition.key);
            const modA = relationships?.[0];
            const modB = relationships?.[1];
            const assertUniqueInBatch = (constrainedForeignKey, constrainedValue, pairValue) => {
                const mapKey = `${constrainedForeignKey}:${constrainedValue}`;
                const existingPairValue = uniquePairsInBatch.get(mapKey);
                if (existingPairValue !== undefined &&
            ...
            if (!modA.hasMany || !modB.hasMany) {
                if (!modA.hasMany && !modB.hasMany) {
                    ...
                    assertUniqueInBatch(modA.foreignKey, link[moduleA][modA.foreignKey], link[moduleB][modB.foreignKey]);
                    assertUniqueInBatch(modB.foreignKey, link[moduleB][modB.foreignKey], link[moduleA][modA.foreignKey]);
                }

...
class MedusaModule {
    ...
    static async onApplicationStart(onApplicationStartCb) {
     ...
     */
    static setSearchIndex(definition, filePath) {
        ...
        if (existing && existing.filePath !== filePath) {
            const describe = (path) => path ?? "the Search Module's options";
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, `Search index "${definition.name}" is defined twice: in ${describe(existing.filePath)} and ${describe(filePath)}`);
        }
        MedusaModule.searchIndexes_.set(definition.name, { definition, filePath });
    }
    static getSearchIndexes() {
        return [...MedusaModule.searchIndexes_.values()].map(({ definition }) => definition);
    }
...
MedusaModule.searchIndexes_ = new Map();
MedusaModule.loading_ = new Map();
MedusaModule.joinerConfig_ = new Map();
MedusaModule.moduleResolutions_ = new Map();
function resolveJoinerConfigDatabaseClientUrl(resolution) {
    const declaration = resolution.moduleDeclaration;
    const options = declaration && typeof declaration === "object" && "options" in declaration
        ? declaration.options
        : resolution.options;
    if (!options) {
        return undefined;
    ...
    try {
        return (0, utils_1.loadDatabaseConfig)(resolution.definition.key, options, true)
            .clientUrl;
    }
    catch {
        return undefined;

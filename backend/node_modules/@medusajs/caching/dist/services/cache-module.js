...
class CachingModuleService {
  ...
  `;
        ...
        await this.strategy.onApplicationStart?.(schema, modules_sdk_1.MedusaModule.getAllJoinerConfigs());
    }
    ...
    async set_({ key, data, ttl, tags, computeAutomaticTags, providers, options, }) {
        if (!key) {
        ...
        const computed = !tags || computeAutomaticTags ? await this.strategy.computeTags(data) : [];
        const tags_ = tags ? (0, utils_1.deduplicate)([...tags, ...computed]) : computed;
        let providers_ = [

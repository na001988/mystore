...
exports.setOrderTaxLinesForItemsStep = (0, workflows_sdk_1.createStep)(exports.setOrderTaxLinesForItemsStepId, async (data, { container }) => {
    const { order, item_tax_lines, shipping_tax_lines, is_full_replacement } = data;
    ...
    const itemsOp = is_full_replacement && itemsTaxLinesData.length
        ? "replace"
        : itemsTaxLinesData.length
            ? "upsert"
            : "none";
    const shippingOp = is_full_replacement && shippingTaxLinesData.length
        ? "replace"
    ...
    const affectedItemIds = itemsOp === "replace" && order.items?.length
        ? order.items.map((i) => i.id)
        : item_tax_lines.map((t) => t.line_item_id);
    const affectedShippingIds = shippingOp === "replace" && order.shipping_methods?.length
    ...
    await (0, utils_1.promiseAll)([
        ...
        shippingOp === "replace"
            ? orderService.setOrderShippingMethodTaxLines(order.id, shippingTaxLinesData)
            : shippingOp === "upsert"
    ...
    const { order, existingLineItemTaxLines, existingShippingMethodTaxLines, affectedItemIds, affectedShippingIds, itemsOp, shippingOp, } = revertData;
    const orderService = container.resolve(utils_1.Modules.ORDER);
    ...
    const shippingTaxLines = (existingShippingMethodTaxLines ?? []).map((taxLine) => ({
        description: taxLine.description,
        tax_rate_id: taxLine.tax_rate_id,
        code: taxLine.code,
        rate: taxLine.rate,
    ...
    const revertShippingTaxLines = async () => {
        if (shippingOp === "replace") {
            await orderService.setOrderShippingMethodTaxLines(order.id, shippingTaxLines);
        }
    ...
    await (0, utils_1.promiseAll)([revertItemTaxLines(), revertShippingTaxLines()]);
});

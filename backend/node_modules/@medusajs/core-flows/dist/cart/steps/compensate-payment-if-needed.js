...
exports.compensatePaymentIfNeededStep = (0, workflows_sdk_1.createStep)(exports.compensatePaymentIfNeededStepId, async (data, { container }) => {
    ...
    if (paymentSession.payment?.captured_at) {
        const { data: [cartPaymentCollectionLink], } = await query.graph({
        ...
        if (cartId) {
            ...
            if (orderCartLink?.order_id) {
                logger.warn(`Skipping refund of payment ${paymentSession.payment.id}: a later completion attempt already placed order ${orderCartLink.order_id} for cart ${cartId}.`);
                return;
            }
        }
        try {

...
const cross_module_joins_1 = require("./cross-module-joins");
 ...
 */
function compileQuery({ query, serviceConfig, options, initialData }, catalog) {
    const { crossModuleJoins, residualCrossModuleFilters, residualOrderBy } = (0, cross_module_joins_1.extractCrossModuleJoins)({ query, serviceConfig }, catalog);
    ...
    const residualHiddenProperties = residualCrossModuleFilters.length || residualOrderBy.length
        ? (0, cross_module_joins_1.consumeResiduals)({
            query,
            serviceConfig,
    ...
    return {
        root: grouped.get(types_1.BASE_PATH),
        expands: grouped,
        shortcuts,
        pkName,
        primaryKeyArg,
...
function parseProperties(params, catalog) {
    ...
    for (const expand of expands) {
        ...
        for (const prop of properties) {
            ...
            const currentPathEntity = parsedExpands.get([types_1.BASE_PATH, ...currentPath].join("."))?.entity ??
                currentServiceConfig.entity;
            const aliasEntry = (0, helpers_1.resolveFieldAliasEntry)(fieldAlias[prop], currentPathEntity);
            if (aliasEntry) {
                const aliasPath = [types_1.BASE_PATH, ...currentPath, prop].join(".");
            ...
            const fullPath = [types_1.BASE_PATH, ...currentPath, prop].join(".");
            const fullAliasPath = [types_1.BASE_PATH, ...currentAliasPath, prop].join(".");
            ...
            const isCurrentProp = fullPath === types_1.BASE_PATH + "." + expand.property ||
                fullAliasPath === types_1.BASE_PATH + "." + expand.property;
            let fields = isCurrentProp ? expand.fields ?? [] : [];
            const args = isCurrentProp ? expand.args : [];
            if (relationship) {
                const parentExpand = parsedExpands.get([types_1.BASE_PATH, ...currentPath].join(".")) ||
                    query;
                if (parentExpand) {
            ...
            if (!parsedExpands.has(fullPath)) {
                let parentPath = [types_1.BASE_PATH, ...currentPath].join(".");
 ...
 */
function rewriteFieldAlias(params, catalog) {
    const { aliasPath, aliasRealPathMap, expands, expand, property, aliasEntry, parsedExpands, currentPath, shortcuts, forwardArgumentsOnPath, } = params;
    ...
    if (aliasRealPathMap.has(aliasPath)) {
        ...
        const existing = [types_1.BASE_PATH, ...currentPath].join(".");
        return parsedExpands.get(existing).serviceConfig;
    }
    const parentPath = [types_1.BASE_PATH, ...currentPath].join(".");
    const parentExpands = parsedExpands.get(parentPath);
    parentExpands.fields = parentExpands.fields?.filter((field) => field !== property);
    forwardArgumentsOnPath.push(...(alias?.forwardArgumentsOnPath || []).map((forPath) => types_1.BASE_PATH + "." + currentPath.concat(forPath).join(".")));
    ...
    for (const mid of middlePath) {
        ...
        if (forwardArgumentsOnPath.includes(types_1.BASE_PATH + "." + midProp)) {
    ...
    currentPath.push(...path.split("."));
    aliasRealPathMap.set(aliasPath, [types_1.BASE_PATH, ...currentPath]);
    return currentServiceConfig;
}
function joinKeyFields(relationship, side) {
    if (side === "parent") {
...
function injectJoinKeys(parentExpand, relationship, _side) {
    parentExpand.fields ??= [];
 ...
 */
function groupSameServiceExpands(parsedExpands) {
 ...
 */
function attachExecutionStages(fullParsedExpands, groupedExpands) {
    const stages = [];
    const rootExp = groupedExpands.get(types_1.BASE_PATH);
    stages.push([
        {
    ...
    const getServiceSequence = (path) => {
        ...
        while (currentPath && currentPath !== types_1.BASE_PATH) {
...
function applySeedFilters(params) {
    ...
    const getPkValues = ({ data, serviceConfig, relationship, }) => {
        ...
        primaryKeys.forEach((key) => {
            filter[key] = Array.from(new Set(data.map((dt) => dt[key]).filter(utils_1.isDefined)));
        });
        return filter;
    };
    ...
    for (let [path, expand] of parsedExpands.entries()) {
        ...
        for (const segment of pathSegments) {
            ...
            relevantInitialData = (0, helpers_1.getNestedItems)(relevantInitialData, segment) ?? [];
        ...
        const queryPath = expand.parent === "" ? types_1.BASE_PATH : aliasToPath ?? path;
...
function getPrimaryKeysAndOtherFilters({ serviceConfig, queryObj, }) {
    ...
    if (!primaryKeyArg) {
        ...
        const primaryKeyFilter = Object.keys(filters).find((key) => serviceConfig.primaryKeys.includes(key));
        if (primaryKeyFilter) {

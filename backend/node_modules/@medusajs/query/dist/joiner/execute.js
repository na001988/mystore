...
const residual_filters_1 = require("./cross-module-joins/residual-filters");
const residual_order_1 = require("./cross-module-joins/residual-order");
 ...
 */
async function executePlan({ plan, dataFetcher, catalog, }) {
    const { options } = plan;
    ...
    if (options?.initialData) {
        data = mergeInitialData({
            ...
            expands: plan.expands.get(types_1.BASE_PATH)?.expands,
            relationship: plan.root.serviceConfig.relationships?.get(plan.root.serviceConfig.serviceName),
            catalog,
        });
    ...
    if (hasResiduals) {
        if (residuals.length) {
            data = (0, residual_filters_1.applyResidualFilters)({ items: data, residuals });
        }
        if (residualOrderBy.length) {
            (0, residual_order_1.sortByResidualOrder)({ items: data, orderBy: residualOrderBy });
        }
        if (plan.residualHiddenProperties?.length) {
            (0, residual_filters_1.hideResidualProperties)({
                items: data,
 ...
 */
function extractRootPagination(root) {
    if (!root.args?.length) {
        return undefined;
    }
    const aliases = {
 ...
 */
function filterFields(data, fields, expands) {
    if (!fields || !data) {
        return data;
    }
    let filteredData = {};
    ...
    if (expands) {
        for (const key of Object.keys(expands)) {
            ...
            if (Array.isArray(data[key])) {
                filteredData[key] = data[key].map((item) => filterFields(item, expand.fields, expand.expands));
            }
            else {
                const filtered = filterFields(data[key], expand.fields, expand.expands);
                if ((0, utils_1.isDefined)(filtered)) {
...
async function fetchData(params) {
    ...
    let uniqueIds;
    if (ids != null) {
        const isIdsUsingOperatorMap = (0, utils_1.isObject)(ids) && Object.keys(ids).some((key) => !!utils_1.FilterOperatorMap[key]);
    ...
    const response = await dataFetcher.fetch(expand, pkFieldAdjusted, uniqueIds, relationship);
    ...
    const filteredDataArray = resData.map((row) => filterFields(row, expand.fields, expand.expands));
    if (isObj) {
...
async function handleExpands(params) {
    ...
    const root = parsedExpands.get(types_1.BASE_PATH);
    ...
    for (const stage of stages) {
        ...
        for (const { paths } of stage) {
            ...
            for (const path of paths) {
                ...
                const { field, fieldsArray, idsToFetch } = computeIdsForRelationship(nestedItems, relationship);
                pathCtx.push({
                    path,
                    expand,
                    relationship,
...
function applyShortcuts(params) {
    ...
    for (const alias of shortcuts) {
        ...
        for (const locationProp of alias.location) {
            itemsLocation = (0, helpers_1.getNestedItems)(itemsLocation, locationProp);
        }
        itemsLocation.forEach((locationItem) => {
            ...
            const curPath = [types_1.BASE_PATH].concat(alias.location);
...
function getItemsForPath(rootItems, fullPath) {
    let nestedItems = rootItems;
    const expandedPathLevels = fullPath.split(".");
    for (let idx = 1; idx < expandedPathLevels.length - 1; idx++) {
        nestedItems = (0, helpers_1.getNestedItems)(nestedItems, expandedPathLevels[idx]);
    }
    return nestedItems;
}
function computeIdsForRelationship(items, relationship) {
...
function createRelatedDataMap(relatedDataArray, joinFields) {
...
function mergeInitialData(params) {
    const { items, initialData, serviceConfig, expands, relationship, catalog } = params;

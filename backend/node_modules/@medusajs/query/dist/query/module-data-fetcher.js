...
exports.ModuleDataFetcher = void 0;
exports.getAllFieldsAndRelations = getAllFieldsAndRelations;
...
class ModuleDataFetcher {
    ...
    async fetch(expand, keyField, ids, relationship) {
        ...
        const processBatch = async (batch) => {
            ...
            try {
                if (ModuleDataFetcher.traceFetchRemoteData) {
                    result = await ModuleDataFetcher.traceFetchRemoteData(async () => service[methodName](batchFilters, options), serviceName, methodName, options);
                }
                else {
    ...
    async executeFetchRequest(params) {
        ...
        let result;
        if (ModuleDataFetcher.traceFetchRemoteData) {
            result = await ModuleDataFetcher.traceFetchRemoteData(async () => service[methodName](filters, options), serviceConfig.serviceName, methodName, options);
        }
        else {
...
exports.ModuleDataFetcher = ModuleDataFetcher;
function getAllFieldsAndRelations(expand, prefix = BASE_PREFIX, args = {}) {
    ...
    for (const property in expand.expands ?? {}) {
        ...
        const result = getAllFieldsAndRelations(expand.expands[property], newPrefix, args);
...
//# sourceMappingURL=module-data-fetcher.js.map
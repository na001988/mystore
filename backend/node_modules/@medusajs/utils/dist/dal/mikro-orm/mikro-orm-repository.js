...
const cross_module_query_1 = require("./cross-module-query");
...
function mikroOrmBaseRepositoryFactory(entity) {
    ...
    class MikroOrmAbstractBaseRepository_ extends MikroOrmBaseRepository {
        ...
        async delete(filters, context) {
            ...
            const primaryKeyColumns = primaryKeys.map((key) => metadata?.properties?.[key]?.fieldNames?.[0] ?? key);
            ...
            return await builder
                ...
                return rows.map((row) => primaryKeys.reduce((acc, key, index) => {
        ...
        async upsertWithReplace(data, config = {
            ...
            const { orderedEntities: upsertedTopLevelEntities, performedActions: performedActions_, } = await this.upsertMany_(manager, this.entity.name, toUpsert);
        ...
        async assignManyToManyRelationBatch_(manager, entitiesData, relation, performedActions, entitiesResults) {
            ...
            if (allNormalizedData.length) {
                ...
                const pivotHasOwnId = pivotMeta?.primaryKeys?.length === 1 &&
                ...
                for (const [parentId, normalizedData] of entityRelationMap) {
                    ...
                    const pivotData = normalizedData.map((currModel) => {
                        ...
                        if (pivotHasOwnId) {
                            const pivotEntry = manager.create(relation.pivotEntity, basePivotEntry, { managed: false, persist: false });
                            return {
                                id: pivotEntry.id,
                                ...basePivotEntry,
                            };
                ...
                const deleteQuery = (manager.getTransactionContext() ?? manager.getKnex())
                    ...
                    .whereNotIn(currentPivotColumn, allPivotData.map((d) => d[currentPivotColumn]));
                if (pivotHasOwnId) {
                    const pivotEntityName = typeof relation.pivotEntity === "string"
                        ? relation.pivotEntity
                        : relation.pivotEntity.name;
                    const [insertResult, deleteResult] = await (0, common_1.promiseAll)([
                        insertQuery.returning("id").execute("all", true),
                        deleteQuery.returning("id"),
                    ]);
                    if (insertResult?.length) {
                        performedActions.created[pivotEntityName] ??= [];
                        performedActions.created[pivotEntityName].push(...insertResult.map((row) => ({ id: row.id })));
                    }
                    if (deleteResult?.length) {
                        performedActions.deleted[pivotEntityName] ??= [];
                        performedActions.deleted[pivotEntityName].push(...deleteResult.map((row) => ({ id: row.id })));
                    }
                }
                else {
                    await (0, common_1.promiseAll)([insertQuery.execute("all", true), deleteQuery]);
                }
            }
        }
        /**
        ...
        async assignCollectionRelation_(manager, data, relation) {
            ...
            if (relation.kind === core_1.ReferenceKind.MANY_TO_MANY) {
                ...
                const deleteQuery = (manager.getTransactionContext() ?? manager.getKnex())
                    ...
                    .from(pivotMeta.tableName)
                    .delete()
                    .where(parentPivotColumn, data.id)
                    .whereNotIn(currentPivotColumn, pivotData.map((d) => d[currentPivotColumn]));
                if (pivotHasOwnId) {
                    const pivotEntityName = typeof relation.pivotEntity === "string"
                        ? relation.pivotEntity
                        : relation.pivotEntity.name;
                ...
                for (let i = 0; i < toUpdate.length; i += batchSize) {
                    ...
                    for (const item of chunk) {
                        for (const prop of onUpdateProps) {
                            item[prop.name] = prop.onUpdate(item, manager);
                        }
                    }
         ...
         */
        getConnectionSchema(manager) {
            const em = manager;
            return em.schema ?? em.config?.get?.("schema") ?? undefined;
        }
        prepareFindOptions(options, manager) {
            let findOptions_ = {
                where: { ...(options.where ?? {}) },
                options: {
                    ...(options.options ?? {}),
            ...
            findOptions_ = (0, cross_module_query_1.augmentFindOptionsWithCrossModuleJoins)(findOptions_, {
            ...
            if (!("strategy" in findOptions_.options)) {
                ...
                Object.assign(findOptions_.options, {
                    strategy: core_1.LoadStrategy.BALANCED,
                });
            }
            ...
            const softDeletableFilter = findOptions_.options.filters?.[mikro_orm_soft_deletable_filter_1.SoftDeletableFilterKey];
            const withDeleted = (0, common_1.isObject)(softDeletableFilter) &&
                softDeletableFilter.withDeleted === true;
            if (!withDeleted) {
                // Override populate to force LEFT JOINs.
                findOptions_.options.populate = this.forceLeftJoinPopulate(findOptions_.options.populate, findOptions_.options.fields);
            }
            return findOptions_;
        }
        /**
         ...
         */
        forceLeftJoinPopulate(populate, fields) {
            let populateHints = populate;
            if ((!Array.isArray(populateHints) || !populateHints.length) &&
                Array.isArray(fields)) {
                const relationPaths = new Set();
                for (const field of fields) {
                    if (!(0, common_1.isString)(field) || !field.includes(".")) {
                        continue;
                    }

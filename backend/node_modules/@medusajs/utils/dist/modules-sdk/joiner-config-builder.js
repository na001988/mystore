...
const belongs_to_1 = require("../dml/relations/belongs-to");
...
function defineJoinerConfig(serviceName, { alias, schema, models, linkableKeys, primaryKeys, idPrefixToEntityName, } = {}) {
    ...
    const crossModuleJoinMetadataByEntity = buildCrossModuleJoinMetadataFromDmlObjects(Array.from(modelDefinitions.values()));
        ...
        alias: [
            ...(alias ?? []).map((aliasConfig) => ({
                ...
                ...(crossModuleJoinMetadataByEntity[aliasConfig.entity]
                    ...
                    : {}),
                args: {
                    methodSuffix: aliasConfig.args?.methodSuffix ??
                        (0, common_1.pluralize)((0, common_1.upperCaseFirst)(aliasConfig.entity)),
                },
            })),
            ...deduplicatedLoadedModels
                ...
                .map((entity) => {
                const entityName = (0, common_1.upperCaseFirst)(entity.name);
                const crossModuleJoinMetadata = crossModuleJoinMetadataByEntity[entityName];
                return {
                    name: [
                    ...
                    ...(crossModuleJoinMetadata
                        ...
                        : {}),
                    args: {
                        methodSuffix: (0, common_1.pluralize)(entityName),
                    },
                };
            }),
        ],
 ...
 */
function buildCrossModuleJoinMetadataFromDmlObjects(models) {
    const metadata = {};
    for (const model of models) {
        if (!dml_1.DmlEntity.isDmlEntity(model)) {
            continue;
        }
        const entityName = (0, common_1.upperCaseFirst)(model.name);
        const { tableNameWithoutSchema, pgSchema } = (0, dml_1.parseEntityName)(model);
        const crossjoinable = [];
        const relations = {};
        for (const [property, value] of Object.entries(model.schema)) {
            if (base_1.BaseRelationship.isRelationship(value)) {
                const relationMetadata = deriveInternalRelationMetadata(model, property, value);
                if (relationMetadata) {
        ...
        metadata[entityName] = {
            crossjoinable: (0, common_1.deduplicate)(crossjoinable),
 ...
 * SQL joins.
 */
function deriveInternalRelationMetadata(model, property, relationship) {
    const parsed = relationship.parse(property);
    const referenced = typeof parsed.entity === "function" ? parsed.entity() : parsed.entity;
    if (!dml_1.DmlEntity.isDmlEntity(referenced)) {
        return undefined;
    }
    const relatedEntityName = (0, common_1.upperCaseFirst)(referenced.name);
    if (parsed.type === "belongsTo") {
        const foreignKey = parsed.options?.foreignKeyName ?? (0, common_1.camelToSnakeCase)(`${property}Id`);
        return {
    ...
    if (parsed.type === "hasMany") {
        ...
        const mappedBy = parsed.mappedBy ??
            (0, common_1.camelToSnakeCase)((0, common_1.upperCaseFirst)((0, common_1.toCamelCase)(model.name)));
        const inverse = referenced.schema[mappedBy];
        if (!inverse || !belongs_to_1.BelongsTo.isBelongsTo(inverse)) {
            return undefined;
        }
        const inverseParsed = inverse.parse(mappedBy);
        const foreignKey = inverseParsed.options?.foreignKeyName ??
            (0, common_1.camelToSnakeCase)(`${mappedBy}Id`);
        return {
            entity: relatedEntityName,
            foreignKey,

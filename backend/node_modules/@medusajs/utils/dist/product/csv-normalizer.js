...
const optionColumns = {
    "variant option": (csvRow, rowColumns, rowNumber, output) => {
        ...
        output["options"] = optionNameColumns.map((columnName) => {
            ...
            if ((0, common_1.isPresent)(id)) {
                entry.id = String(id);
            }
            if ((0, common_1.isPresent)(isExclusiveRaw)) {
                const parsed = (0, common_1.tryConvertToBoolean)(isExclusiveRaw);
                if (parsed !== undefined) {
...
_CSVNormalizer_rows = new WeakMap(), _CSVNormalizer_products = new WeakMap(), _CSVNormalizer_instances = new WeakSet(), _CSVNormalizer_getOrInitializeProductById = function _CSVNormalizer_getOrInitializeProductById(id) {
    ...
    options.options.forEach(({ key, value, id, is_exclusive }) => {
        ...
        if (!matchingKey) {
            const newOption = { title: key, values: [value] };
            if (id !== undefined) {
                newOption.id = id;
            }
            ...
            if (matchingKey.is_exclusive === undefined &&

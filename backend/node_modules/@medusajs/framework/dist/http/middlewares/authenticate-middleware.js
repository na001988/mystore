...
const authenticate = (actorType, authType, options = {}) => {
    const authenticateMiddleware = async (req, res, next) => {
        ...
        if (authTypes.includes(API_KEY_AUTH) &&
            ...
            res.status(401).json({
                message: "A secret API key was passed as a Bearer token. Secret API keys must be sent using HTTP Basic authentication instead (Authorization: Basic <secret-api-key>).",
            });
            return;
        }
...
const isBearerSecretApiKey = (authHeader) => {
    if (!authHeader) {
        return false;
    }
    const [tokenType, token] = authHeader.split(" ");
    ...
    if (matches) {
        ...
        if (tokenType.toLowerCase() === BEARER_AUTH) {
            ...
            try {
                ...
                if (!Object.prototype.hasOwnProperty.call(options, "ignoreExpiration")) {

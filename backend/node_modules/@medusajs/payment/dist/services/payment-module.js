...
class PaymentModuleService extends utils_1.ModulesSdkUtils.MedusaService(generateMethodForModels) {
    ...
    async authorizePaymentSession(id, context, sharedContext) {
        ...
        if (status === utils_1.PaymentSessionStatus.PENDING_AUTHORIZATION) {
    ...
    async capturePayment(data, sharedContext = {}) {
        ...
        const capturedPayment = await this.paymentService_.retrieve(payment.id, { relations: ["captures"], options: { refresh: true } }, sharedContext);
    ...
    async capturePayment_(data, payment, sharedContext = {}) {
        ...
        else if (utils_1.MathBN.lte(data.amount, 0)) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, `Capture amount must be greater than 0.`);
        }
        ...
        if (!knex) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.UNEXPECTED_STATE, "capturePayment_ must run inside a transaction to serialize concurrent captures.");
        }
        ...
        const lockedPayment = await this.paymentService_.retrieve(data.payment_id, { select: ["id"], relations: ["captures.raw_amount"] }, sharedContext);
    ...
    async refundPayment_(payment, data, sharedContext = {}) {
        ...
        if (data.amount == null) {
            ...
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, `Refund amount must be greater than 0.`);
        }
        ...
        const knex = manager?.getTransactionContext();
        if (!knex) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.UNEXPECTED_STATE, "refundPayment_ must run inside a transaction to serialize concurrent refunds.");
        }
        // Bound the wait for the row lock so a contended refund fails fast instead
        // of hanging: `SET LOCAL` scopes the timeout to this transaction.
        await knex.raw("SET LOCAL lock_timeout = '3s'");
        await knex("payment").where("id", payment.id).forUpdate().select("id");
        const lockedPayment = await this.paymentService_.retrieve(data.payment_id, {
            select: ["id"],
            relations: ["captures.raw_amount", "refunds.raw_amount"],
        }, sharedContext);
        const capturedAmount = lockedPayment.captures.reduce((captureAmount, next) => {

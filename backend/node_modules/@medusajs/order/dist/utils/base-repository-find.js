 ...
 */
function applyCrossModuleJoins(repository, findOptions, manager) {
    const entityName = repository.entity.name ?? repository.entity.meta?.className ?? "Order";
    const primaryKey = utils_1.MikroOrmBaseRepository.retrievePrimaryKeys(repository.entity)[0] ??
        "id";
    const defaultSchema = manager.schema ?? manager.config?.get?.("schema") ?? undefined;
    const augmented = (0, utils_1.augmentFindOptionsWithCrossModuleJoins)(findOptions, {
        entityName,

...
const isEmptyFilterValue = (value) => value === undefined ||
    value === null ||
    value === "" ||
    (Array.isArray(value) && value.length === 0);
const DataTableFilterBar = ({ clearAllFiltersLabel = "Clear all", alwaysShow = false, children, }) => {
        ...
        setLocalFilters((prevLocalFilters) => {
            ...
            const updatedLocalFilters = prevLocalFilters
                ...
                if (parentIds.includes(f.id)) {
                    ...
                    if (!(0, lodash_isequal_1.default)(f.value, parentValue) || f.isNew !== isNew) {
                        return {
                            id: f.id,
                            value: parentValue,
                            isNew,
    ...
    return (React.createElement("div", { className: "flex w-full flex-nowrap items-center justify-between gap-2 overflow-x-auto border-t px-6 py-2" },
        React.createElement("div", { className: "flex min-h-[28px] flex-nowrap items-center gap-2 md:flex-wrap" },
            ...
            filterCount > 0 && (React.createElement(button_1.Button, { variant: "transparent", size: "small", type: "button", className: "text-ui-fg-muted hover:text-ui-fg-subtle flex-shrink-0 whitespace-nowrap", onClick: clearFilters }, clearAllFiltersLabel))),
        children && (React.createElement("div", { className: "flex flex-shrink-0 items-center gap-2" }, children))));
};
...
const DataTableFilterBarSkeleton = ({ filterCount, }) => {
    return (React.createElement("div", { className: "flex w-full overflow-x-auto border-t px-6 py-2" },
        React.createElement("div", { className: "flex min-h-[28px] flex-nowrap items-center gap-2 md:flex-wrap" },

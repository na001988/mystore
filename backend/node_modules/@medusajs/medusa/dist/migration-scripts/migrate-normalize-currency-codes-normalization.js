...
exports.default = migrateNormalizeCurrencyCodes;
const utils_1 = require("@medusajs/framework/utils");
const workflows_sdk_1 = require("@medusajs/framework/workflows-sdk");
const retrieveNonNormalizedCurrencyCodesStep = (0, workflows_sdk_1.createStep)(`retrieve-non-normalized-currency-codes`, async ({ table }, { container }) => {
...
const updateCurrencyCodesStep = (0, workflows_sdk_1.createStep)(`update-currency-codes`, async ({ records, table }, { container }) => {
    if (!records.length) {
        return new workflows_sdk_1.StepResponse(0, { records, table });
    }
    const knex = container.resolve(utils_1.ContainerRegistrationKeys.PG_CONNECTION);
    const response = await knex(table)
        .whereIn("id", records.map((r) => r.id))
        .update({ currency_code: knex.raw("LOWER(currency_code)") });
    return new workflows_sdk_1.StepResponse(response, { records, table });
}, async (data, { container }) => {
    if (!data)
        return;
    const { records, table } = data;
    const knex = container.resolve(utils_1.ContainerRegistrationKeys.PG_CONNECTION);
    await (0, utils_1.promiseAll)(records.map((r) => knex(table).where("id", r.id).update({ currency_code: r.currency_code })));
});
const updateIndexDataStep = (0, workflows_sdk_1.createStep)("update-index-data-currency-codes", async (_, { container }) => {
    const knex = container.resolve(utils_1.ContainerRegistrationKeys.PG_CONNECTION);
    ...
    return new workflows_sdk_1.StepResponse(void 0, prevData);
}, async (data, { container }) => {
    if (!data) {
        return;
    }
    const knex = container.resolve(utils_1.ContainerRegistrationKeys.PG_CONNECTION);
    await (0, utils_1.promiseAll)(data.map(async (d) => {
...
const migrateNormalizeCurrencyCodesWorkflow = (0, workflows_sdk_1.createWorkflow)("migrate-normalize-currency-codes", () => {
...
async function migrateNormalizeCurrencyCodes({ container, }) {
...
//# sourceMappingURL=migrate-normalize-currency-codes-normalization.js.map
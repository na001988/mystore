...
async function default_1({ types, directory, lint, fix }) {
    ...
    await (0, lint_project_1.runLintStep)({
        directory,
        lint,
        fix,
        quiet: true,
     ...
     */
    const startArgs = args.filter((arg) => !/^--(no-)?(lint|fix)(=.*)?$/.test(arg));
    if (types) {
    ...
    const devServer = {
        ...
        async start() {
            ...
            this.childProcess = (0, child_process_1.fork)(cliPath, ["start", ...startArgs], forkOptions);

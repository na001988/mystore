...
function applyPromotionToItems(promotion, items, appliedPromotionsMap, allocationOverride) {
    ...
    if (allocation === utils_1.ApplicationMethodAllocation.ACROSS) {
        lineItemsAmount = applicableItems.reduce((acc, item) => {
            ...
            const appliedInBase = promotion.is_tax_inclusive
                ? utils_1.MathBN.mult(appliedExclTax, utils_1.MathBN.div(item.original_total, item.subtotal))
                : appliedExclTax;
            return utils_1.MathBN.sub(utils_1.MathBN.add(acc, itemBase), appliedInBase);
    ...
    for (const item of applicableItems) {
        ...
        const appliedAmountExclTax = promotion.is_tax_inclusive
            ? utils_1.MathBN.mult(amount, utils_1.MathBN.div(item.subtotal, item.original_total))
            : amount;
        appliedPromotionsMap.set(item.id, utils_1.MathBN.add(appliedPromoValue, appliedAmountExclTax));
        if (allocation === utils_1.ApplicationMethodAllocation.ONCE) {

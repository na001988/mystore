...
async function crawl(dir, file, depth) {
  ...
  const crawler = new fdir().withBasePath().exclude((dirName) => dirName.startsWith("_")).filter((path5) => {
...
function getConfigObjectProperties(path5) {
  if (isVariableDeclarator(path5.node)) {
    const configDeclaration = isIdentifier(path5.node.id, { name: "config" }) ? path5.node : null;
...
async function generateCellRendererHash(sources) {
  ...
  for (const source of sources) {
    const cellRenderersPath = path2.join(source, "cell-renderers.tsx");
    try {
...
function getConfigArgument(path5) {
  if (!isCallExpression(path5.node.declaration)) {
    return null;
  }
  if (!isIdentifier(path5.node.declaration.callee, {
...
async function getCustomFieldContents(file) {
  const code = await fs4.readFile(file, "utf-8");
  let ast = null;
  try {
...
async function generateI18nHash(sources) {
  ...
  const contents = await Promise.all(
    indexFiles.map((file) => fs6.readFile(file, "utf-8"))
  );
  const totalContent = contents.join("");
  return generateHash(totalContent);
}
...
async function generateLayoutHash(sources) {
  const files = await getLayoutFilesFromSources(sources);
  const contents = await Promise.all(
    files.map((file) => fs8.readFile(file, "utf-8"))
  );
...
async function parseFile5(file, index) {
  const code = await fs11.readFile(file, "utf-8");
  let ast = null;
  try {
    ast = parse(code, getParserOptions(file));
  } catch (e) {
...
async function getWidgetContents(file) {
  const code = await fs12.readFile(file, "utf-8");
  let ast;
  try {
    ast = parse(code, getParserOptions(file));
  } catch (e) {
...
async function parseFile6(file, index) {
  const code = await fs13.readFile(file, "utf-8");
  let ast;
  try {
    ast = parse(code, getParserOptions(file));
  } catch (e) {
...
async function generateVirtualCellRendererModule(sources, pluginMode = false) {
  const imports = [];
  for (const source of sources) {
    const cellRenderersPath = path3.join(source, "cell-renderers.tsx");
    try {
      await fs14.access(cellRenderersPath);
      imports.push(`import "${cellRenderersPath}"`);
    } catch {
    }
  }
  const code = outdent15`
    ${imports.join("\n")}
...
var medusaVitePlugin = (options) => {
  ...
  function isFileInSources(file) {
    for (const source of _sources) {
      if (file.startsWith(path4.resolve(source))) {
        return true;
      }
    }
    return false;
  ...
  const pluginEntryFile = path4.resolve(
    process.cwd(),

...
async function crawl(dir, file, depth) {
  ...
  const crawler = new import_fdir.fdir().withBasePath().exclude((dirName) => dirName.startsWith("_")).filter((path5) => {
    return VALID_FILE_EXTENSIONS.some((ext) => path5.endsWith(ext));
  });
  if (file) {
    crawler.filter((path5) => {
      return VALID_FILE_EXTENSIONS.some((ext) => path5.endsWith(file + ext));
    });
  }
  if (depth) {
...
function getConfigObjectProperties(path5) {
  if ((0, import_types.isVariableDeclarator)(path5.node)) {
    const configDeclaration = (0, import_types.isIdentifier)(path5.node.id, { name: "config" }) ? path5.node : null;
  ...
  const declaration = path5.node.declaration;
...
async function hasDefaultExport(ast) {
  ...
  traverse(ast, {
    ...
    AssignmentExpression(path5) {
      if (path5.node.left.type === "MemberExpression" && path5.node.left.object.type === "Identifier" && path5.node.left.object.name === "exports" && path5.node.left.property.type === "Identifier" && path5.node.left.property.name === "default") {
        hasDefaultExport2 = true;
      }
    },
...
// src/cell-renderers/generate-cell-renderer-hash.ts
async function generateCellRendererHash(sources) {
  const contents = [];
  for (const source of sources) {
    const cellRenderersPath = import_path2.default.join(source, "cell-renderers.tsx");
    try {
      const content = await import_promises.default.readFile(cellRenderersPath, "utf-8");
...
function getModel(path5, file) {
  const configArgument = getConfigArgument(path5);
  if (!configArgument) {
    return null;
  }
...
function getConfigArgument(path5) {
  if (!(0, import_types.isCallExpression)(path5.node.declaration)) {
    return null;
  }
  if (!(0, import_types.isIdentifier)(path5.node.declaration.callee, {
    name: "unstable_defineCustomFieldsConfig"
  })) {
    return null;
  }
  const configArgument = path5.node.declaration.arguments[0];
  if (!(0, import_types.isObjectExpression)(configArgument)) {
    return null;
  }
  return configArgument;
}
function validateLink(path5, file) {
  const configArgument = getConfigArgument(path5);
  if (!configArgument) {
    return false;
  }
...
async function parseDisplayFile(file, index) {
  ...
  try {
    traverse(ast, {
      ExportDefaultDeclaration(path5) {
        const _model = getModel(path5, file);
        if (!_model) {
          return;
        }
        model = _model;
        displays = getDisplays(path5, model, index, file);
        hasLink = validateLink(path5, file);
      }
    });
  } catch (err) {
...
function getDisplays(path5, model, index, file) {
  const configArgument = getConfigArgument(path5);
  if (!configArgument) {
    return null;
  }
...
function generateImport(file, index) {
  const path5 = normalizePath(file);
  return `import ${generateCustomFieldConfigName(index)} from "${path5}"`;
}
  ...
  try {
    traverse(ast, {
      ExportDefaultDeclaration(path5) {
        ...
        model = _model;
        hasLink = validateLink(path5, file);
        configs = getConfigs(path5, model, index, file);
        forms = getForms(path5, model, index, file);
      }
    });
  } catch (err) {
    logger.error(`An error occurred while traversing the file.`, {
...
function generateImport2(file, index) {
  const path5 = normalizePath(file);
  return `import ${generateCustomFieldConfigName2(index)} from "${path5}"`;
}
function getForms(path5, model, index, file) {
  const formArray = getFormsArgument(path5, file);
  if (!formArray) {
    return null;
  }
  const forms = [];
...
function getConfigs(path5, model, index, file) {
  const formArray = getFormsArgument(path5, file);
  if (!formArray) {
...
function getFormsArgument(path5, file) {
  const configArgument = getConfigArgument(path5);
  if (!configArgument) {
    return null;
  }
...
async function getCustomFieldContents(file) {
  const code = await import_promises4.default.readFile(file, "utf-8");
  let ast = null;
  try {
...
async function parseFile2(file, index) {
  const content = await import_promises5.default.readFile(file, "utf8");
  let ast;
  try {
    ast = (0, import_parser.parse)(content, getParserOptions(file));
  } catch (e) {
...
function generateImport3(file, index) {
  const path5 = normalizePath(file);
  return `import ${generateCustomFieldConfigName3(index)} from "${path5}"`;
}
function getLink(path5, index, file) {
  const configArgument = getConfigArgument(path5);
  if (!configArgument) {
    return null;
  }
...
async function generateI18nHash(sources) {
  ...
  const contents = await Promise.all(
    indexFiles.map((file) => import_promises6.default.readFile(file, "utf-8"))
  );
  const totalContent = contents.join("");
  return generateHash(totalContent);
}
...
// src/layouts/generate-layouts.ts
var import_promises7 = __toESM(require("fs/promises"));
var import_outdent3 = __toESM(require("outdent"));

// src/layouts/helpers.ts
async function getLayoutFilesFromSources(sources) {
  return (await Promise.all(
...
// src/layouts/generate-layouts.ts
async function generateLayouts(sources) {
...
async function getLayoutResults(files) {
  return (await Promise.all(files.map(parseFile3))).filter(
    (r) => r !== null
  );
}
function generateCode3(results) {
  return import_outdent3.default`
    layouts: [
      ${results.map(
    ({ componentName, configName }) => `{ ...${configName}, Component: ${componentName} }`
  ).join(",\n      ")}
    ]
  `;
}
async function parseFile3(file, index) {
  ...
  return {
    import: generateImport4(file, componentName, configName),
    componentName,
    configName
  };
}
function hasConfigExport(ast) {
  ...
  traverse(ast, {
    ExportNamedDeclaration(path5) {
      ...
      if (getConfigObjectProperties(path5)) {
        found = true;
      }
    }
  });
...
function generateImport4(file, componentName, configName) {
  const path5 = normalizePath(file);
  return `import ${componentName}, { config as ${configName} } from "${path5}"`;
}

// src/layouts/generate-layout-hash.ts
var import_promises8 = __toESM(require("fs/promises"));
async function generateLayoutHash(sources) {
  const files = await getLayoutFilesFromSources(sources);
  const contents = await Promise.all(
    files.map((file) => import_promises8.default.readFile(file, "utf-8"))
  );
...
function formatMenuItem(route) {
  const { label, icon, path: path5, nested, rank, translationNs } = route;
  return `{
    label: ${label},
    icon: ${icon || "undefined"},
    path: "${path5}",
...
async function getMenuItemResults(files) {
  const results = await Promise.all(files.map(parseFile4));
...
function generateImport5(file, index) {
  const path5 = normalizePath(file);
  return `import { config as ${generateRouteConfigName(index)} } from "${path5}"`;
}
  ...
  try {
    traverse(ast, {
      ...
      VariableDeclarator(path5) {
        ...
        const properties = getConfigObjectProperties(path5);
        if (!properties) {
          return;
        }
...
async function getRouteContents(file) {
  ...
  try {
    traverse(ast, {
      ExportDefaultDeclaration(path5) {
        defaultExportContent = code.slice(path5.node.start, path5.node.end);
      },
      ExportNamedDeclaration(path5) {
        const properties = getConfigObjectProperties(path5);
        if (properties) {
          configContent = code.slice(path5.node.start, path5.node.end);
        }
      }
    });
  } catch (e) {
...
function generateCode5(results) {
  return import_outdent5.outdent`
        routes: [
...
async function getRouteResults(files) {
  const results = (await Promise.all(files.map(parseFile5))).filter(
    (result) => result !== null
  );
...
async function parseFile5(file, index) {
  const code = await import_promises11.default.readFile(file, "utf-8");
  let ast = null;
  try {
    ast = (0, import_parser.parse)(code, getParserOptions(file));
  } catch (e) {
...
function generateImports(file, index, hasHandle, hasLoader, hasConfig) {
  const imports = [];
  ...
  if (!hasHandle && !hasLoader && !hasConfig) {
    ...
    const namedImports = [
      ...
      hasConfig && `config as ${generateRouteConfigName2(index)}`
    ].filter(Boolean).join(", ");
    imports.push(`import ${route}, { ${namedImports} } from "${importPath}"`);
  }
  return imports;
}
function generateRoute(route, index, hasHandle, hasLoader, hasConfig) {
  return {
...
function generateHandle(index, hasHandle, hasConfig) {
  ...
  const configName = generateRouteConfigName2(index);
  const labelFields = `label: ${configName}.label, translationNs: ${configName}.translationNs`;
  return hasHandle ? `{ ${labelFields}, ...${generateHandleName(index)} }` : `{ ${labelFields} }`;
}
...
function generateRouteConfigName2(index) {
...
// src/virtual-modules/generate-virtual-layout-module.ts
var import_outdent8 = __toESM(require("outdent"));
async function generateVirtualLayoutModule(sources, pluginMode = false) {
  const layouts = await generateLayouts(sources);
  const imports = [...layouts.imports];
  const code = import_outdent8.default`
    ${imports.join("\n")}

    ${pluginMode ? `const layoutModule = { ${layouts.code} }` : `export default { ${layouts.code} }`}
  `;
  return generateModule(code);
}
...
async function generateVirtualRouteModule(sources, pluginMode = false) {
  ...
  const code = import_outdent11.outdent`
    ${imports.join("\n")}
...
async function getWidgetContents(file) {
  const code = await import_promises12.default.readFile(file, "utf-8");
  let ast;
  try {
    ast = (0, import_parser.parse)(code, getParserOptions(file));
  } catch (e) {
...
async function generateWidgets(sources) {
  ...
  const imports = results.map((r) => r.import);
  const code = generateCode6(results);
  return {
    imports,
    code
  };
...
async function getWidgetResults(files) {
  return (await Promise.all(files.map(parseFile6))).filter(
    (r) => r !== null
  );
}
function generateCode6(results) {
  return import_outdent12.default`
    widgets: [
...
function getWidgetId(idOverride, file) {
  if (idOverride) {
    return idOverride;
  }
  ...
  const relative = markerIndex >= 0 ? normalized.slice(markerIndex + 1) : normalized;
  return `Widget-${generateHash(relative).slice(0, 4)}`;
}
async function parseFile6(file, index) {
  const code = await import_promises13.default.readFile(file, "utf-8");
  let ast;
  try {
    ast = (0, import_parser.parse)(code, getParserOptions(file));
  } catch (e) {
  ...
  const import_ = generateImport6(file, index);
...
function generateImport6(file, index) {
  ...
  return `import ${generateWidgetComponentName(
    index
  )}, { config as ${generateWidgetConfigName(index)} } from "${path5}"`;
}
...
function getWidgetIdOverride(ast) {
  ...
  const readId = (path5) => {
    ...
    const idProperty = properties.find(
      (p) => p.type === "ObjectProperty" && p.key.name === "id"
    );
...
async function getWidgetZone(ast, file) {
  ...
  traverse(ast, {
    ...
    VariableDeclarator(path5) {
      ...
      if (path5.node.id.type === "Identifier" && path5.node.id.name === "config" && path5.node.init?.type === "CallExpression") {
        const arg = path5.node.init.arguments[0];
...
async function generateVirtualWidgetModule(sources, pluginMode = false) {
  ...
  const code = import_outdent13.default`
    ${imports.join("\n")}
...
async function generateVirtualI18nModule(sources, pluginMode = false) {
  ...
  const code = import_outdent14.default`
        ${imports.join("\n")}
...
async function generateVirtualCellRendererModule(sources, pluginMode = false) {
  const imports = [];
  for (const source of sources) {
    const cellRenderersPath = import_path3.default.join(source, "cell-renderers.tsx");
    try {
      await import_promises14.default.access(cellRenderersPath);
      imports.push(`import "${cellRenderersPath}"`);
    } catch {
    }
  }
  const code = import_outdent15.default`
    ${imports.join("\n")}

    ${pluginMode ? `const cellRendererModule = {}` : `export default {}`}
  `;
  return generateModule(code);
}
  ...
  return {
    ...
    async buildStart() {
      switch (mode) {
        case "plugin" /* PLUGIN */: {
          ...
          await (0, import_promises15.writeFile)(pluginEntryFile, code, "utf-8");
          break;
        }
    ...
    async buildEnd() {
      switch (mode) {
        case "plugin" /* PLUGIN */: {
          try {
            await (0, import_promises15.rm)(pluginEntryFile, { force: true });
          } catch (error) {
          }
          break;
        }

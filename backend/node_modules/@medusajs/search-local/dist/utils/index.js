...
exports.FILTER_SUFFIX = exports.SOURCE_KEY = void 0;
exports.assertIndexSupported = assertIndexSupported;
exports.assertQuerySupported = assertQuerySupported;
exports.buildIndexPlan = buildIndexPlan;
exports.sameSchema = sameSchema;
exports.toOramaDocument = toOramaDocument;
exports.projectDocument = projectDocument;
exports.toOramaWhere = toOramaWhere;
exports.extractPrimaryKeyFilter = extractPrimaryKeyFilter;
exports.toOramaFacets = toOramaFacets;
exports.fromOramaFacets = fromOramaFacets;
 ...
 */
exports.FILTER_SUFFIX = "__filter";
...
function isFacetable(field) {
    return field.facetable === true || typeof field.facetable === "object";
}
function scalarType(field, isPrimaryKey) {
    if (isPrimaryKey) {
    ...
    switch (field.type) {
        ...
        case "geo":
            return "geopoint";
        default:
            return fail(`The local search provider does not support fields of type "${field.type}"`);
    }
}
/**
 ...
 */
function assertIndexSupported(definition) {
    const walk = (group, prefix) => {
        for (const [name, field] of Object.entries(group)) {
            const path = prefix ? `${prefix}.${name}` : name;
            if (field.correlated) {
                fail(`The local search provider cannot correlate predicates per element, so "${path}" cannot set "correlated". Arrays of objects are collapsed to per-leaf arrays.`);
            }
            if (field.type === "vector") {
                fail(`The local search provider does not support vector fields ("${path}")`);
            }
 ...
 */
function assertQuerySupported(query) {
    const sortFields = Object.keys(query.pagination?.order ?? {}).filter((key) => key !== "_score");
    if (sortFields.length > 1) {
        fail(`The local search provider sorts on one field at a time; got ${sortFields.length} (${sortFields.join(", ")})`);
    }
    ...
    if (options.locales?.length) {
        fail("The local search provider does not support query-time locales; matching is language-agnostic");
    }
 ...
 */
function buildIndexPlan(definition) {
    const schema = {};
    const fields = new Map();
    const searchable = [];
    const walk = (group, prefix, inArray) => {
        for (const [name, field] of Object.entries(group)) {
            const path = prefix ? `${prefix}.${name}` : name;
            ...
            const type = (isArray && scalar !== "geopoint" ? `${scalar}[]` : scalar);
            const planned = {
                path,
                type,
                is_array: isArray,
            ...
            if (tokenized && (field.filterable || isFacetable(field))) {
                planned.filter_path = `${path}${exports.FILTER_SUFFIX}`;
...
function sameSchema(a, b) {
    ...
    if (keysA.some((key, i) => key !== keysB[i] || a.schema[key] !== b.schema[key])) {
        return false;
    }
    ...
    return (searchableA.length === searchableB.length &&
...
function toNumber(value, isDate) {
    const num = isDate
...
function toOramaDocument(document, plan) {
    const target = {};
    ...
    if (primaryKey === undefined || primaryKey === null || primaryKey === "") {
        fail(`A document written to the local search provider is missing its primary key "${plan.primary_key}"`);
    }
    // Orama takes the document id from `id`.
    target.id = String(primaryKey);
    target[exports.SOURCE_KEY] = document;
    return target;
}
// Narrows a source document to the requested dotted paths.
function projectDocument(source, paths) {
    const picked = {};
    for (const path of paths) {
        ...
        if (Array.isArray(value)) {
            ...
            picked[head] = value.map((entry, i) => Object.assign(existing[i] ?? {}, projectDocument(entry, [rest.join(".")])));
        }
        else if (value && typeof value === "object") {
            picked[head] = Object.assign(picked[head] ?? {}, projectDocument(value, [rest.join(".")]));
        }
    }
    return picked;
}
function resolveFilterTarget(path, plan) {
...
function toOramaOperation(path, raw, planned, type) {
    const isArrayType = type.endsWith("[]");
    const base = type.replace("[]", "");
    const cast = (value) => coerce(value, planned);
    ...
    if (raw === null || typeof raw !== "object" || Array.isArray(raw)) {
        ...
        if (base === "number") {
            if (values.length > 1) {
                fail(`The local search provider cannot match a numeric field ("${path}") against several values`);
            }
            return { eq: values[0] };
        }
        if (isArrayType) {
    ...
    const unsupported = (reason) => fail(`The local search provider cannot apply ${keys.join(" + ")} to "${path}": ${reason}`);
    ...
    if (keys.length !== 1) {
        unsupported("the local search provider accepts one operation per field; only $gte with $lte can be combined");
    }
    const [operator] = keys;
...
// rejected.
function toOramaWhere(filters, plan) {
    if (!filters) {
        return undefined;
    }
    const where = {};
    const merge = (branch) => {
        for (const [key, value] of Object.entries(branch)) {
            ...
            if (key === "$or" || key === "$not") {
                ...
                fail(`The local search provider cannot express ${key} in filters; only conjunctions ($and) are supported`);
            }
            const { key: target, planned, type } = resolveFilterTarget(key, plan);
            if (target in where) {
                fail(`The local search provider cannot apply two conditions to "${key}" in one query`);
            }
 ...
 */
function extractPrimaryKeyFilter(filters, plan) {
    ...
    if (operators[0] === "$eq" || operators[0] === "$in") {
        return asIds(predicate[operators[0]]);
    }
    return undefined;
}
/* --------------------------------- facets --------------------------------- */
...
function toOramaFacets(requests, plan) {
    if (!requests?.length) {
        return undefined;
    }
    const orama = {};
    const byOramaKey = new Map();
...
function fromOramaFacets(facets, plan) {
    const result = {};

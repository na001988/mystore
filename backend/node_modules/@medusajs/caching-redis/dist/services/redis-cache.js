...
class RedisCachingProvider {
        ...
        if (key) {
            try {
                ...
                if (this.isConnectionError(error)) {
                    this.logger.warn(`[redis-cache] Redis connection error during get operation, returning empty array to trigger fallback to original data source. Error: ${error?.message ?? error}`);
                    return null;
                }
                throw error;
            }
    ...
    async set({ key, data, ttl, tags, options, }) {
        try {
            ...
            if (this.isConnectionError(error)) {
                this.logger.warn(`[redis-cache] Redis connection error during set operation, relying on IORedis retry mechanism. Error: ${error?.message ?? error}`);
                return;
            }
            throw error;
        }
    ...
    async clear({ key, tags, options, }) {
        try {
            ...
            if (this.isConnectionError(error)) {
                this.logger.warn(`[redis-cache] Redis connection error during clear operation, relying on IORedis retry mechanism. Error: ${error?.message ?? error}`);
                return;
            }
            throw error;
        }
    ...
    async flush() {
        try {
            ...
            if (this.isConnectionError(error)) {
                this.logger.warn(`[redis-cache] Redis connection error during flush operation, relying on IORedis retry mechanism. Error: ${error?.message ?? error}`);
                return;
            }
            throw error;
        }

...
exports.buildOptionValueFilterQuery = buildOptionValueFilterQuery;
 ...
 */
async function buildOptionValueFilterQuery(optionValueIds, sharedContext) {
    ...
    const optionIds = optionGroups.map((row) => row.option_id);
    const allValueIds = optionGroups
        .map((row) => row.option_value_ids)
        .flat();
  ...
  `.trim();
    return {
        [(0, postgresql_1.raw)((alias) => subquery(alias))]: true,
    };
}
//# sourceMappingURL=build-option-value-filter-query.js.map
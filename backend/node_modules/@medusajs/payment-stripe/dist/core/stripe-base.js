...
class StripeBase extends utils_1.AbstractPaymentProvider {
    ...
    normalizePaymentIntentParameters(extra) {
        const res = {};
        const paymentMethodTypes = extra?.payment_method_types ??
        ...
        if (!paymentMethodTypes?.length) {
            res.payment_method_configuration =
                extra?.payment_method_configuration ??
    ...
    getStatus(paymentIntent) {
        switch (paymentIntent.status) {
            ...
            case "requires_confirmation":
                return { status: utils_1.PaymentSessionStatus.PENDING, data: paymentIntent };
            case "processing":
                if (this.isAsyncPaymentMethod(paymentIntent.payment_method)) {
                    return {
                        status: utils_1.PaymentSessionStatus.PENDING_AUTHORIZATION,
                        data: paymentIntent,
                    };
                }
                return { status: utils_1.PaymentSessionStatus.PENDING, data: paymentIntent };
    ...
    async getWebhookActionAndData(webhookData) {
        ...
        switch (event.type) {
            ...
            case "payment_intent.processing":
                ...
                const paymentMethod = typeof webhookPaymentMethod === "object"
                    ? intent.payment_method
                    : typeof webhookPaymentMethod === "string"
                        ? await this.stripe_.paymentMethods.retrieve(webhookPaymentMethod)
                        : null;
                return {
                    action: paymentMethod && this.isAsyncPaymentMethod(paymentMethod)
                        ? utils_1.PaymentActions.PENDING_AUTHORIZATION
                        : utils_1.PaymentActions.PENDING,
                    data: {
    ...
    isAsyncPaymentMethod(paymentMethod) {
        return (this.options_.asyncPaymentMethodTypes?.includes(paymentMethod.type) ??
            false);
    }
    /**
